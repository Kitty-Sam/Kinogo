...
import { AddNewOrderPayloadType, fetchOrders, removeOrder } from '~store/sagas/sagasActions';
...
export const PastScreen = () => {
    ...
    const renderOrderItem = useCallback(
        ({ item }: { item: AddNewOrderPayloadType }) => (
    ...
    return (
        <ScreenContainer bgColor={bgColor}>
            <ListContainer>
                <FlatList data={currentOrders} renderItem={renderOrderItem} keyExtractor={(item) => item.id} />
            </ListContainer>
        </ScreenContainer>
    );
};
